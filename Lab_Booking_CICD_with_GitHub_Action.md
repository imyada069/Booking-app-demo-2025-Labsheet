...
### ขั้นตอนที่ 3.3: ติดตั้ง Dependencies และรัน Migrations

```bash
# ติดตั้ง Node.js dependencies
npm install

# สร้าง Prisma client (auto-generate จาก schema)
npx prisma generate
...
**คำอธิบาย**:
- `npx prisma generate`: สร้าง Prisma Client ที่ type-safe ตาม schema ที่กำหนด
- `npx prisma migrate dev`: สร้าง SQL migration files และ apply กับ database อัตโนมัติ
- `--name init`: ชื่อ migration (ตั้งชื่อให้สื่อความหมาย)

**ตรวจสอบ Database Structure** (ไม่บังคับ):
...
### ขั้นตอนที่ 3.4: Start Backend Server
